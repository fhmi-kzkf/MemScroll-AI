...
! function(t, e) {
    ...
    function Aa(t, e, r, i) {
        ...
        }, H.push(p), p.sd && (p.sd < 0 && (p.b = d, p.a = n), J && fa(p.b, z ? et[z] : nt), T && H.push(p.swap = {
            t: n.element,
            b: p.b,
            a: p.a,
            sd: -p.sd,
        ...
        z && (st[z] || ba(z)).forEach(function(e) {
            ...
            for (ea(H), za(!0), l = 0; l < H.length; l++) p = H[l], g = p.a, v = p.b, !D || g.isDifferent(v) || p.entering ? (u = p.t, !L || p.sd < 0 || !l || (g.matrix = getGlobalMatrix(u, !1, !1, !0)), v.isVisible && g.isVisible ? (p.sd < 0 ? (h = new pt(u, z, t.simple), ot(h, g, A, 0, 0, h), h.matrix = getGlobalMatrix(u, !1, !1, !0), h.css = p.b.css, p.a = g = h, T && (u.style.opacity = J ? v.opacity : g.opacity), O && q.push(u)) : 0 < p.sd && T && (u.style.opacity = J ? g.opacity - v.opacity : "0"), ot(g, v, A, z)) : v.isVisible !== g.isVisible && (v.isVisible ? g.isVisible || (v.css = g.css, j.push(v), H.splice(l--, 1), S && L && ot(g, v, A, z)) : (g.isVisible && G.push(g), H.splice(l--, 1))), A || (u.style.maxWidth = Math.max(g.width, v.width) + "px", u.style.maxHeight = Math.max(g.height, v.height) + "px", u.style.minWidth = Math.min(g.width, v.width) + "px", u.style.minHeight = Math.min(g.height, v.height) + "px"), L && I && u.classList.add(I)) : H.splice(l--, 1), f.push(g);
            var e;
    ...
    var ht = (Flip.getState = function getState(t, e) {
        ...
        }), a && ja(d, f), n = ot(d, f, s || c, l, c, n.duration || o ? n : 0), "object" == typeof i && "zIndex" in i && (n.zIndex = i.zIndex), g && !o && g.add(function() {
            return function() {
                return ga(d)
            }
    ...
    function Flip() {}
    ht.version = "3.12.7", "undefined" != typeof window && window.gsap && window.gsap.registerPlugin(ht), e.Flip = ht, e.default = ht;
